package com.azavea.rf.datamodel.v16.schema
...
trait tables {
  ...
  case class FootprintsRow(id: java.util.UUID, organizationId: java.util.UUID, createdAt: java.sql.Timestamp, modifiedAt: java.sql.Timestamp, multipolygon: Projected[Geometry], scene: java.util.UUID)
  ...
  class Footprints(_tableTag: Tag) extends Table[FootprintsRow](_tableTag, "footprints") {
    def * = (id, organizationId, createdAt, modifiedAt, multipolygon, scene) <> (FootprintsRow.tupled, FootprintsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(organizationId), Rep.Some(createdAt), Rep.Some(modifiedAt), Rep.Some(multipolygon), Rep.Some(scene)).shaped.<>({r=>import r._; _1.map(_=> FootprintsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Foreign key referencing Scenes (database name footprints_scene_fkey) */
    lazy val scenesFk = foreignKey("footprints_scene_fkey", scene, Scenes)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
   ...
   *  @param createdBy Database column created_by SqlType(varchar), Length(255,true)
   *  @param modifiedBy Database column modified_by SqlType(varchar), Length(255,true)
   *  @param rawDataBytes Database column raw_data_bytes SqlType(int4)
   *  @param visibility Database column visibility SqlType(visibility)
   *  @param filename Database column filename SqlType(text)
   *  @param sourceuri Database column sourceuri SqlType(text)
   *  @param scene Database column scene SqlType(uuid)
   *  @param bands Database column bands SqlType(_text), Length(2147483647,false)
   *  @param imageMetadata Database column image_metadata SqlType(jsonb), Length(2147483647,false) */
  case class ImagesRow(id: java.util.UUID, createdAt: java.sql.Timestamp, modifiedAt: java.sql.Timestamp, organizationId: java.util.UUID, createdBy: String, modifiedBy: String, rawDataBytes: Int, visibility: Visibility, filename: String, sourceuri: String, scene: java.util.UUID, bands: List[String], imageMetadata: Map[String, Any])
  /** GetResult implicit for fetching ImagesRow objects using plain SQL queries */
  implicit def GetResultImagesRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Timestamp], e2: GR[String], e3: GR[Int], e4: GR[Visibility], e5: GR[List[String]], e6: GR[Map[String, Any]]): GR[ImagesRow] = GR{
    prs => import prs._
    ImagesRow.tupled((<<[java.util.UUID], <<[java.sql.Timestamp], <<[java.sql.Timestamp], <<[java.util.UUID], <<[String], <<[String], <<[Int], <<[Visibility], <<[String], <<[String], <<[java.util.UUID], <<[List[String]], <<[Map[String, Any]]))
  }
  ...
  class Images(_tableTag: Tag) extends Table[ImagesRow](_tableTag, "images") {
    ...
    /** Database column modified_by SqlType(varchar), Length(255,true) */
    val modifiedBy: Rep[String] = column[String]("modified_by", O.Length(255,varying=true))
    /** Database column raw_data_bytes SqlType(int4) */
    ...
    /** Database column sourceuri SqlType(text) */
    val sourceuri: Rep[String] = column[String]("sourceuri")
    /** Database column scene SqlType(uuid) */
    val scene: Rep[java.util.UUID] = column[java.util.UUID]("scene")
    /** Database column bands SqlType(_text), Length(2147483647,false) */
    val bands: Rep[List[String]] = column[List[String]]("bands", O.Length(2147483647,varying=false))
    /** Database column image_metadata SqlType(jsonb), Length(2147483647,false) */
    ...
    lazy val usersFk3 = foreignKey("images_created_by_fkey", createdBy, Users)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
    /** Foreign key referencing Users (database name images_modified_by_fkey) */
    lazy val usersFk4 = foreignKey("images_modified_by_fkey", modifiedBy, Users)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
