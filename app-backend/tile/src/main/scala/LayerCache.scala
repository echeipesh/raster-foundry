...
object LayerCache extends Config with LazyLogging {
  ...
  def layerHistogram(layerId: UUID, zoom: Int): OptionT[Future, Array[Histogram[Double]]] = OptionT {
    histogramCache.caching(s"histogram-$layerId-$zoom") { implicit ec =>
  ...
  def layerTile(layerId: UUID, zoom: Int, key: SpatialKey): OptionT[Future, MultibandTile] = OptionT {
    tileCache.caching(s"tile-$layerId-$zoom-${key.col}-${key.row}") { implicit ec =>
  ...
  def layerTileForExtent(layerId: UUID, zoom: Int, extent: Extent): OptionT[Future, MultibandTile] = OptionT {
    tileCache.caching(s"extent-tile-$layerId-$zoom-$extent") { implicit ec =>
      attributeStoreForLayer(layerId).mapFilter { store =>
        Try {
